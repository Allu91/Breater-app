...
window.addEventListener('DOMContentLoaded', () => {
    ...
    topnavInfo.addEventListener("click", () => {
        ...
        let appInfoCopy = document.querySelector("#app_info_content_copy");
        appInfoCopy.innerHTML = new Date().getFullYear();
    })

    // Close modal if close button or outside of modal clicked
    appInfo.addEventListener("click", (event) => {
        if (!appInfoWpr.contains(event.target) || event.target == appInfoClose) {
